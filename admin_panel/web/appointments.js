...
async function fetchAppointments(status) {
  ...
  try {
    ...
    if (allAppointments.length === 0) {
      appointmentsList.innerHTML =
        '<tr><td colspan="7">No appointments found for the selected status.</td></tr>';
      return;
    }
    ...
    appointmentsList.innerHTML =
      '<tr><td colspan="7">Error loading appointments. Please try again later.</td></tr>';
  }
}
...
document
  ...
  .addEventListener("click", () => filterAppointmentsByStatus("upcoming"));
document
  .getElementById("filterCompleted")
  .addEventListener("click", () => filterAppointmentsByStatus("completed"));
document
  .getElementById("filterMissed")
  .addEventListener("click", () => filterAppointmentsByStatus("missed"));
document
  .getElementById("filterToday")
  .addEventListener("click", () => filterAppointmentsByStatus("today"));
